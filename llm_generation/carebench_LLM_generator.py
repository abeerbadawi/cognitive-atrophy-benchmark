"""
5-Model AI Response Generator — CareBench Dataset
───────────────────────────────────────────────────
Models  : Qwen, Llama, GPT, Claude, Gemini
Input   : topic*.xlsx (column: "input", multi-turn patient–assistant dialogues)
Context : Full conversation history passed as messages (no truncation)
Output  : one CSV per conversation (topic<N>_results.csv) + all_results.csv
"""
import os
import re
import time
...
# --- Folders ---
INPUT_DIR  = "ADD_YOUR_INPUT_DIR_HERE"   # topic1.xlsx ... topic255.xlsx
OUTPUT_DIR = "ADD_YOUR_OUTPUT_DIR_HERE"         # one CSV per conversation saved here

# --- Shared params ---
MAX_TOKENS        = 2048  # high ceiling so no model cuts off mid-sentence
GPT_MAX_TOKENS    = 2048  # GPT counts input+output combined
MAX_WORDS         = 300   # all outputs trimmed to same word count for fairness
MAX_HISTORY_TURNS = 10    # last N turns passed as context
TEMPERATURE       = 1.0
TOP_P             = 1.0
...
def trim_response(text: str) -> str:
    """Trim all model outputs to MAX_WORDS for fair comparison."""
...
# ─────────────────────────────────────────────────────────────────
def load_conversation(filepath: str) -> list[str]:
    df = pd.read_excel(filepath)
    inputs = df["input"].dropna().tolist()
    return inputs
...
def process_conversation(conv_idx, filepath, clients, model_config):
    conv_name = os.path.splitext(os.path.basename(filepath))[0]  # e.g. "topic1"
    ...
    inputs = load_conversation(filepath)
    histories = {"qwen": [], "llama": [], "openai": [], "claude": [], "gemini": []}
    results   = []
    conv_start = time.time()

    for turn_idx, user_input in enumerate(inputs, start=1):
        turn_start = time.time()
        print(f"  Turn {turn_idx:02d}/{len(inputs)} | {time.strftime('%H:%M:%S')} | {user_input[:60].replace(chr(10),' ')}...")
        ...
        def call_model(args):
            ...
            response = call_fn(client, user_input, histories[hkey])
    ...
    print(f"  ✅ {conv_name} done — {len(inputs)} turns in {conv_elapsed}s → {output_csv}")
    return df
...
def main():
    print("\n" + "="*65)
    print("  5-Model Batch Generator  |  Mental Health Dialogue")
    ...
    # Find all conversation files sorted numerically
    files = sorted(
        glob.glob(os.path.join(INPUT_DIR, "topic*.xlsx")),
        key=lambda x: int(re.search(r'\d+', os.path.basename(x)).group())
    )

    if not files:
        print(f"❌ No topic*.xlsx files found in {INPUT_DIR}")
        return
