"""
5-Model AI Response Generator — PAIR Dataset
──────────────────────────────────────────────
Dataset : Single-turn counseling prompts
          prompt → AI responds (no history, no turns)
...
OUTPUT_CSV      = os.path.join(OUTPUT_DIR, "pair_results.csv")
CHECKPOINT_FILE = os.path.join(OUTPUT_DIR, "pair_checkpoint.csv")
...
def main():
    print("\n" + "="*65)
    print("  5-Model Generator  |  PAIR Dataset  |  Single-Turn")
    ...
    df_input = df_input.reset_index()   # use index as row ID
    df_input.rename(columns={"index": "rowID"}, inplace=True)
    print(f"📂 Loaded {len(df_input)} rows from {INPUT_FILE}")

    # Check for existing checkpoint to resume
    if os.path.exists(CHECKPOINT_FILE):
        done_df  = pd.read_csv(CHECKPOINT_FILE)
        done_ids = set(done_df["rowID"].tolist())
    ...
    for _, input_row in df_input.iterrows():
        row_id      = input_row["rowID"]
        prompt      = str(input_row["prompt"]).strip()
        label       = str(input_row.get("label", ""))
        subcategory = str(input_row.get("subcategory", ""))
        hq1         = str(input_row.get("hq1", ""))
        hq2         = str(input_row.get("hq2", ""))
        ...
        row_start = time.time()
        print(f"\n  [{row_id+1}/{total}] | {prompt[:70].replace(chr(10),' ')}...")
        ...
        row = {
            ...
            "hq1"       : hq1,
            "hq2"       : hq2,
            "hq1 words" : str(input_row.get("hq1 words", "")),
            "hq2 words" : str(input_row.get("hq2 words", "")),
        }
